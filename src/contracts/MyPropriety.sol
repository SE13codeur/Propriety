...
    function _sellingPropriety(uint256 _id, uint256 _price) external isOwner(_id) {            
        proprieties[_id].price = _price;            
        proprieties[_id].state = StateProperty.ON_SALE;
    }

    function buyingPropriety(uint256 _id) payable external {
        require(proprieties[_id].state == StateProperty.ON_SALE);            require(proprieties[_id].price > 0);            require(proprieties[_id].price == msg.value);
        payable(proprieties[_id].owner).transfer(msg.value);                        emit Sale(_id, proprieties[_id].owner, msg.sender, block.timestamp);
        changeOwner(_id, payable(msg.sender));
    }

    function changeOwner(uint256 _id, address payable _assignee) private {        
...
    function officialPropriety(uint256 _id) payable external isOwner(_id) {                
